...
# declare csv headers
x = np.genfromtxt('data/votes_train.csv', delimiter=',', dtype=str)  # use training set

# enumerate parameters and instantiate Imputer
imp = Imputer()


def missing_data_cond(x): return x == '?'
...
print('imputing with predicted values using logistic regression')
...
def compute_histogram(data, labels):
    histogram = np.unique(sorted(data), return_counts=True)
    histogram = np.transpose(np.asarray(histogram))
    for label in labels:
...
freq_data = {'Raw data': compute_histogram(x[:, 1], labels),
             ...
             'PCA': compute_histogram(data_facanal[:, 1], labels),
             'KNN': compute_histogram(data_knn[:, 1], labels)}
